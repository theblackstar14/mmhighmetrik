export default function CambiosAlcanceLoading() {
  ...
  return (
    <>
      ...
      <div style={{ padding: '14px 26px', borderBottom: '1px solid #E2E8F0', background: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div><div style={{ ...shimmer, width: 140, height: 18, marginBottom: 6 }} /><div style={{ ...shimmer, width: 200, height: 11 }} /></div>
        <div style={{ ...shimmer, width: 130, height: 34 }} />
      </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E2E8F0', overflow: 'hidden' }}>
          {Array.from({length: 6}).map((_,i) => (
            <div key={i} style={{ padding: '12px 16px', borderBottom: '1px solid #F1F5F9', display: 'flex', gap: 16 }}>
              {[50,60,160,200,90,80,90].map((w,j) => <div key={j} style={{ ...shimmer, width: w, height: 14 }} />)}
            </div>
          ))}
        </div>
      </div>
